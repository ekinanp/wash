...
func (s *ObjectTestSuite) TestValueInDomain_ElementPredicate() {
	p := Object()
	s.MUM(p, s.A("object", s.A(s.A("key", "fOo"), true)))
	s.VIDFTC(p, "foo", true, []interface{}{}, map[string]interface{}{}, map[string]interface{}{"foo": nil}, map[string]interface{}{"bar": true})
	...
	for _, key := range []string{"foo", "FOO", "foO"} {
		s.VIDTTC(p, map[string]interface{}{key: true})
	}
}
...
func (s *ObjectTestSuite) TestEvalValue_ElementPredicate() {
	...
	// Test with different keys to ensure that the object predicate finds the first matching key
	for _, key := range []string{"foo", "FOO", "foO"} {
		s.EVFTC(p, map[string]interface{}{key: false})
		s.EVTTC(p, map[string]interface{}{key: true})
	}
}

func (s *ObjectTestSuite) TestExpression_AtomAndNot_ElementPredicate() {
	expr := expression.New("object", func() rql.ASTNode {
		return Object()
	})

	s.MUM(expr, s.A("object", s.A(s.A("key", "foo"), true)))
	s.VIDFTC(expr, "foo")
	s.VIDTTC(expr, map[string]interface{}{"foo": false})
	s.EVFTC(expr, "foo", true, []interface{}{}, map[string]interface{}{}, map[string]interface{}{"foo": nil}, map[string]interface{}{"bar": true}, map[string]interface{}{"foo": false})
	s.EVTTC(expr, map[string]interface{}{"foo": true})
	s.MUM(expr, s.A("NOT", s.A("object", s.A(s.A("key", "foo"), true))))
	s.VIDFTC(expr, "foo")
	s.VIDTTC(expr, map[string]interface{}{"foo": true})
	s.EVTTC(expr, map[string]interface{}{"foo": false})
	s.EVFTC(expr, "foo", true, []interface{}{}, map[string]interface{}{}, map[string]interface{}{"foo": nil}, map[string]interface{}{"bar": true}, map[string]interface{}{"foo": true})
