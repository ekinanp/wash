...
func stringifySupportedAttributes(path string, entry apitypes.Entry) string {
	...
	metadataLines := []string{
		...
		fmt.Sprintf("to view the (partial) metadata."),
	}
	...
	for _, action := range actions {
		...
		case plugin.ListAction().Name:
			actionDescriptionLines = []string{
				...
				fmt.Sprintf("- find %s <expression>", path),
				fmt.Sprintf("    Prints all children that satisfy <expression>. Type 'find --help' to"),
				fmt.Sprintf("    see all the things that you can filter on."),
